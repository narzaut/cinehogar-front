...
export function UserInput({icon, type, errors, register, value, placeholder, required }) {
	...
	const 	reg = register(value, {required: {value:required, message:'Campo requerido'}, pattern: {value:/^\S+@\S+$/i, message:'Email inválido'}})
	...
	return (
		<div className='py-3 '>
			...
			<p className='text-shadow pt-1 pl-1 text-xs text-red-500 font-bold '>{error ? <span><i className="fas fa-exclamation-circle"></i> {error}</span>  : ''}</p>
		</div>
  );
}
